...
public class ToolLayerMove extends Tool {
	...
	public ToolLayerMove(Window window, GroupNodeWrapper wrapper, GroupNodeEditHandle editHandle) {
		...
		CustomBinding.bindBidirectional(wrapper.specificChild, new SelectionModelBinder<>(layerList.getSelectionModel()));
		CustomBinding.bind(origin.visibleProperty(), wrapper.specificChild.map(s -> opt(s != null)));
		new DoubleHalfBinder<>(wrapper.specificChild,wrapper.getConfig().frame).addListener((select, frame) -> {
			if (originCleanup != null) {
				originCleanup.destroy();
				originCleanup = null;
			}
			if (select == null) return;
			pos = GroupChildWrapper.positionFrameFinder.findFrame(select,
