...
public abstract class BaseFrameRowAdapter<N, F> extends RowAdapter {
	...
	@Override
	final public boolean duplicateFrame(
	...
	@Override
	final public boolean createFrame(
			ProjectContext context, Window window, ChangeStepBuilder change, int outer
	) {
		int inner = window.timeToInner(outer);
		if (inner == NO_INNER)
	...
	public boolean createFrame(
			ProjectContext context, ChangeStepBuilder change, int inner, FrameFinder.Result<F> previous, F newFrame
	) {
	...
	@Override
	final public boolean frameAt(Window window, int outer) {
	...
	@Override
	final public boolean hasFrames() {
		return true;
	}
	...
	@Override
	final public int updateTime(ProjectContext context, Window window) {
		return row.map(r -> {
