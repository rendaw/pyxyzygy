		...
		tabBox.getChildren().addAll(new TitledPane("Layer",
				...
				new WidgetFormBuilder()
						...
						.intSpinner("Width", 1, 99999, s -> {
							cleanup.add(CustomBinding.bindBidirectional(new CustomBinding.PropertyBinder<Integer>(
									...
									new CustomBinding.PropertyBinder<Integer>(s
											.getValueFactory()
											.valueProperty())
							));
						})
				...
				new WidgetFormBuilder().apply(b -> cleanup.add(nodeFormFields(context,
					...
					d
							...
							.addListener((observable, oldValue, newValue) -> wrapper.config.renderMode = newValue);
					...
					t
							.textProperty()
							.addListener((observable, oldValue, newValue) -> wrapper.config.renderName = newValue);
					...
					button.setOnAction(e -> uncheck(() -> {
						...
						switch (wrapper.config.renderMode) {
							...
							case WEBM: {
								...
								Picture rgbPic = Picture.create(wrapper.node.width() * scale,
							...
							case GIF:
								try (
									...
									GifEncoder gifEncoder =
											new GifEncoder(stream, wrapper.node.width(), wrapper.node.height(), 0);
									...
									Color[][] rgb = new Color[wrapper.node.height() * scale][wrapper.node.width() *
											scale];
									wrapper.render(context, window, (i, canvas) -> {
										byte[] bgra = canvas.data();
										for (int y = 0; y < wrapper.node.height(); ++y) {
							...
							case PNG_SEQUENCE: {
								wrapper.render(context,
										window,
										(i, canvas) -> canvas.serialize(dir
												.resolve(String.format("%s%06d.png", wrapper.config.renderName, i))
												.toString()),
										scale
								);
							}
