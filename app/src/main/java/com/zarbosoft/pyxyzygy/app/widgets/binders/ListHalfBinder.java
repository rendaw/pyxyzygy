...
public class ListHalfBinder<T> implements HalfBinder<List<T>> {
  ...
  public ListHalfBinder(Object target, String prop) {
    if (Character.isUpperCase(prop.charAt(0))) throw new Assertion();
    ...
    listenRemove =
        bindConsumer(target, "add" + camel + "RemoveListeners", Listener.ListRemove.class);
    unlistenRemove =
        bindConsumer(target, "remove" + camel + "RemoveListeners", Listener.ListRemove.class);
    listenMoveTo =
        bindConsumer(target, "add" + camel + "MoveToListeners", Listener.ListMoveTo.class);
    unlistenMoveTo =
        bindConsumer(target, "remove" + camel + "MoveToListeners", Listener.ListMoveTo.class);
  }
  ...
  public BinderRoot addListener(Consumer<List<T>> listener) {
    final Listener.ListAdd listAdd =
        (target, at, value) -> {
          listener.accept(get.get());
        };
    listenAdd.accept(listAdd);
    final Listener.ListRemove listRemove =
        (target, at, count) -> {
          listener.accept(get.get());
        };
    listenRemove.accept(listRemove);
    final Listener.ListMoveTo listMoveTo =
