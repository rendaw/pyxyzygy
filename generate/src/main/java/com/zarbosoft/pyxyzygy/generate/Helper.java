...
public class Helper extends Task {
  ...
  public static boolean flattenPoint(TypeInfo type) {
    if (ProjectObject.class.isAssignableFrom((Class) type.type)) return true;
    return false;
  }
  ...
  public static TypeName toPoetBoxed(
      TypeInfo info, Map<Class, Pair<ClassName, TypeSpec.Builder>> typeMap) {
    final Class type =
        ((Supplier<Class>)
                () -> {
            ...
            .get();
    TypeName base =
        Optional.ofNullable(typeMap.get(type))
            .map(p -> (TypeName) p.first)
            .orElseGet(() -> TypeName.get(type));
    if (info.parameters != null && info.parameters.length > 0) {
      return ParameterizedTypeName.get(
          (ClassName) base,
          (TypeName[])
              Arrays.stream(info.parameters).map(p -> toPoet(p, typeMap)).toArray(TypeName[]::new));
    } else return base;
  }
