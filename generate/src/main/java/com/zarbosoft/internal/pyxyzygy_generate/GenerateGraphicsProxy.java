...
import static com.zarbosoft.internal.pyxyzygy_generate.GenerateTask.poetMethod;
import static com.zarbosoft.internal.pyxyzygy_generate.GenerateTask.write;
...
public class GenerateGraphicsProxy {
	public static void generateGraphicsProxy(Path path) {
		...
		ClassName name = ClassName.get("com.zarbosoft.pyxyzygy.widgets", "NearestNeighborGraphics");
		...
		Arrays.stream(graphics.getMethods()).forEach(method -> {
			...
			b.addMethod(poetMethod(method, ImmutableMap.of()).addCode(code.build()).build());
		});
		write(path, name, b.build());
	}
}
